 ...
 * @since      1.0.0
 */
class enjoytube_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoytube-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'enjoytube' )
		);

		// Create the widget.
		parent::__construct(
			'enjoytube-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'enjoytube' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'enjoytube_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoytube_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoytube_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'enjoytube' ),
			'limit'     => 5,
			'show_date' => true
		);
