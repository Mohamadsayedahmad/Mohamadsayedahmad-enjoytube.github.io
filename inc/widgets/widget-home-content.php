 ...
 * @since      1.0.0
 */
class EnjoyTube_Home_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoytube-home-block-one',
			'description' => __( 'Display post content blocks. Only use for the "Home Content" widget area.', 'enjoytube' )
		);

		// Create the widget.
		parent::__construct(
			'enjoytube-home-block-one',          // $this->id_base
			__( '&raquo; Home Content', 'enjoytube' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			$query = apply_filters( 'enjoytube_home_one_column_args', $args );
				...
				<div class="content-block content-block-1 clear">
					...
					<?php
						if ( ( ! empty( $instance['title'] ) ) && ( $cat_id != 0 ) ) {
							...
							echo '<span class="section-more-link"><a href="' . esc_url( $cat_link ) . '">'. esc_html( 'More', 'enjoytube' ) . '</a></span>';

						} elseif ( $cat_id == 0 ) {

							echo '<h3 class="section-title"><span>' . wp_kses_post( apply_filters( 'widget_title',  esc_html( 'Latest Posts', 'enjoytube' ), $instance, $this->id_base ) ) . '</span></h3>';
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>">
				<?php esc_html_e( 'Title:', 'enjoytube' ); ?>
			</label>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Choose category', 'enjoytube' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'cat' ) ); ?>" style="width:100%;">
				...
				<option value="0"><?php esc_html_e( 'All categories &hellip;', 'enjoytube' ); ?></option>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
				...
				<?php esc_html_e( '3 Columns Layout', 'enjoytube' ); ?>		    	
			</label>		    
		<br/>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	...
		    	<?php esc_html_e( '4 Columns Layout', 'enjoytube' ); ?>
			</label>		    
	    </p>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'enjoytube' ); ?>
			</label>
