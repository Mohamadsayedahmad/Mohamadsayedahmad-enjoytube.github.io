 ...
 * @since      1.0.0
 */
class enjoytube_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoytube-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'enjoytube' )
		);

		// Create the widget.
		parent::__construct(
			'enjoytube-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'enjoytube' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'enjoytube_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoytube_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoytube_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'enjoytube' ),
			'limit'     => 5,
			'show_date' => true
		);
